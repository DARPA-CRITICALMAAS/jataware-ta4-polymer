...
@router.get("/cdr/d/{doc_id}")
async def index2(doc_id: str):
    return RedirectResponse(f"/cdr/d/{doc_id}/1")


@router.get("/cdr/d/{doc_id}/{page1}")
async def index(doc_id: str, page1: int, request: Request, bbox: str = ""):
    if page1 < 1:
        return RedirectResponse(f"/cdr/d/{doc_id}/1")
    try:
        meta = cdrops.get_doc_meta(doc_id)
    except Exception:
        # todo not found?
        raise
    # cache
    try:
        ...
        return RedirectResponse(f"/cdr/download/progress/{doc_id}?page={page1}")
...
@router.get("/partials/cdr/d/img/{doc_id}/{page1}/{xref}", response_class=Response)
async def index_overview_image(doc_id: str, page1: int, xref: int, request: Request):
    doc = cache_open_pdf(doc_id)
    img = next((img for img in doc[page1 - 1].get_images() if img[0] == xref), None)
    if not img:
...
@router.post("/doc/partials/clip/{doc_id}/{page1}")
async def clip_item(doc_id: str, page1: int, rects: Rects, request: Request):
    with db_session() as session:
        pdf = session.query(DbPdf).filter_by(id=doc_id).one()
    ...
    for r in rects.selections:
        ...
        img_url = f"/svg/pdf/{doc_id}/{page1}/{x0}/{y0}/{x1}/{y1}"
        p.set_cropbox(wrect)
    ...
    return templates.TemplateResponse(
        "doc/items.html", {"request": request, "doc_id": doc_id, "file": pdf.file_name, "page": page1, "clips": clips}
    )
...
@router.get("/partial/cdr/doc/annotations/{doc_id}/{page1}")
def doc_annotations(doc_id: str, page1: int, request: Request):
    with db_session() as session:
        annotations = (
            session.query(DbAnnotation)
            .options(joinedload(DbAnnotation.tags))
            .filter_by(doc_id=doc_id, page=page1 - 1)
            .all()
        )
...
@router.delete("/partial/cdr/doc/annotations/{annotation_id}")
...
@router.get("/cdr/png/pdf/{doc_id}/{page1}", response_class=Response)
def png(request: Request, doc_id: str, page1: int, term: str = "", blocks: bool = False, bbox: str = ""):
    doc = cache_open_pdf(doc_id)
    p = doc[page1 - 1]
    blue = (0, 0, 1)
    red = (1, 0, 0)
...
@router.get("/cdr/png/pdf/{doc_id}/{page1}/{x0}/{y0}/{x1}/{y1}", response_class=Response)
def png_crop(request: Request, doc_id: str, page1: int, x0: float, y0: float, x1: float, y1: float):
    doc = cache_open_pdf(doc_id)
    p = doc[page1 - 1]
...
@router.get("/cdr/thumb/pdf/{doc_id}/{page1}", response_class=Response)
def png_thumb(request: Request, doc_id: str, page1: int, term: str = "", blocks: bool = False, bbox: str = ""):
    doc = cache_open_pdf(doc_id)
    p = doc[page1 - 1]
    blue = (0, 0, 1)
    red = (1, 0, 0)
    ...
    return Response(content=pix.tobytes(), media_type="image/png", headers=headers)


@router.get("/cdr/info/d/{doc_id}")
def doc_info(request: Request, doc_id: str):
    ...
    return templates.TemplateResponse("cdr_doc/metadata.html", {"request": request, "content": "", "doc": doc})


@router.post("/partials/cdr/info/meta/{doc_id}")
def doc_info_metadata(
    request: Request,
    doc_id: str,
    authors: Annotated[List[str], Form(..., default_factory=list)],
    ...
    res = httpx.post(
        url, headers=headers, json=meta.model_dump(exclude_none=True), params={"title": title}, timeout=None
    )
...
@router.post("/partials/cdr/info/prov/{doc_id}")
def doc_info_add_provenance(
    request: Request,
    doc_id: str,
    provname: Annotated[str, Form()] = "",
    provid: Annotated[str, Form()] = "",
    provurl: Annotated[str, Form()] = "",
):
    token = app_settings.cdr_api_key
    headers = {"Authorization": f"Bearer {token}"}
    ...
    if name or id or url:
        ...
        res = httpx.post(cdrurl, headers=headers, json=prov.model_dump(exclude_none=True), timeout=None)
        logger.debug(res.status_code)
        logger.debug(res.content)
        res.raise_for_status()

        return templates.TemplateResponse(
            "cdr_doc/provenance_list_info.html",
            {"request": request, "content": "", "p": prov.model_dump(exclude_none=True)},
        )
