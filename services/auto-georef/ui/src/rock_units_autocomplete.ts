...
const MAJOR_TYPES = ["Major1", "Major2", "Major3", "Minor1", "Minor2", "Minor3", "Minor4", "Minor5"];
    ...
    return fetch(`${window.rock_units_uri}?major_type=${mt}`)
      ...
      .then((mtValues) => {
        if (Array.isArray(mtValues)) {
          ...
          console.error("Rock unit detail response is not an array, not setting to session storage. Details:", mtValues);
        }
      });
  });
...
async function prepareAutocomplete() {
  ...
  MAJOR_TYPES.forEach(mt => {
    ...
    const formatted = JSON.parse(sessionStorage.getItem(`${STORAGE_KEY_PREFIX}${mt}`))
      .map(label => ({label: label, value: label, group: mt}));

    Array.prototype.push
      .apply(rock_units, formatted)
  });
...
function addRockUnitBadge(selection) {
  const newElem = !selectedItems.find(si => si.value === selection.value && si.group === selection.group);
  if (newElem) {
...
function renderRockUnitBadges() {
  ...
  document.querySelectorAll(".rock_unit_badge_delete")
    .forEach(badge_x => {
  ...
  selectedItems.forEach((i, idx) => {
    ...
    newBadge.className = "badge text-slate-500 dark:text-slate-400 rounded-sm border-slate-400/25 badge-ghost badge-outline badge-xs mb-1";
...
function updateFormInputs() {
  ...
  for (let i of MAJOR_TYPES) {
    ...
    document.querySelector(`input[name=sgmc_geology_${formattedType}]`).value = JSON.stringify(formatted_selected[i]);
  }
}
...
export const initAutocomplete = async (elem, containerElem) => {
  ...
  autocomplete({
    ...
    renderGroup: function(groupName, currentValue) {
      ...
      div.className = "text-white bg-primary dark:text-neutral-color dark:bg-neutral pl-1 border-bottom border-neutral border-solid";
