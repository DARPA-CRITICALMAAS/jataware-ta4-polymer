...
const fetchCMAs = ()  => fetch(window.list_cmas_uri)
  .then(r => r.json())
  .then(
    CMAs => CMAs.map(cma => ({
      label: cma.mineral,
...
export const initAutocomplete = async (elem, valueElem, containerElem) => {
  ...
  autocomplete({
    ...
    onSelect: function(item) {
      ...
      drawMultiPolygonOpenLayers(window.polymer_map, {
      ...
      if (item.crs !== cdrDefaultProjection) {
        CDR_multiPolygonCoordinates = CDR_multiPolygonCoordinates.map(polygon => {
          return transformPolygonCoordinates(
            polygon,
            // We trust "known" set of valid crs up-front, under ./geo.ts:
            item.crs,
  ...
  elem.addEventListener("blur", e => {
    setTimeout(() => {
      ...
      if (!valueElem.dataset.label || (valueElem.dataset.label.toLowerCase() !== elem.value.toLowerCase())) {
        elem.value = "";
        valueElem.value = null;
        valueElem.dataset.label = "";
        // clear previous ol area preview
