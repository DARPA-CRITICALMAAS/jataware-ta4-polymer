 ...
 **/
async function readZipFileAs(zipFileReferenceObj, readAs:FileOutput=FileOutput.Text) {
...
async function readShapeFile(allShapeData) {
  ...
  if (!prjFile) {
    ...
    const noPrjError = new Error(`Unable to detect projection. Missing .prj file for shape: ${shpFile.filename}`);
...
function parsePolygon(shape, coordinates, from_prj, to_prj, formatFn) {
  let updated_coordinates = transformPolygonCoordinates(coordinates, from_prj, to_prj);
    ...
    (acc, curr) => {
      ...
      shapes.forEach((shape) => {
        ...
        if (type === "MultiPolygon") {
          const updated_coordinates = coordinates
                .map(c => parsePolygon(shape, c, prj, to_prj, formatFn))
          acc = [
            ...acc,
            ...updated_coordinates // spread these individual Polygons; we explode MultiPolygons into Polygon features
          ]
        } else if (type === "Polygon") {
          const updated_coordinates = parsePolygon(shape, coordinates, prj, to_prj, formatFn);
          acc = [
            ...acc,
            updated_coordinates
          ]
...
function parseCoordinatesForCDR(shapeArrays) {
  const multiPolygonCoordinates = parseShapeCoordinates(shapeArrays, cdrDefaultProjection);
...
function parseCoordinatesForOpenLayers(shapeArrays) {
  ...
  const draw_features = parseShapeCoordinates(shapeArrays, openLayersDefaultProjection, formatFn);

  if (!draw_features.length) {
    const noPolygonsError = new Error("Shapefile zip did not contain any Polygons.");
  ...
  const firstCoordinate = draw_features[0].geometry.coordinates[0][0];
  const view = window.polymer_map.getView();
  view.setCenter(firstCoordinate);
  // TODO base zoom on dbf data sqkm
  view.setZoom(5.8);
}
...
export function groupShapeFiles(zipFileEntries) {
  const shape_entries = zipFileEntries.filter(f => f.filename.endsWith(".shp"));

  return shape_entries.map(sf_dataFile => {

    // remove folder path + extension to match other corresponding files by name
    const shp_name_without_ext = stripFilePath(sf_dataFile.filename).replace(/\.shp$/,'');

    const dbf_dataFile = zipFileEntries.find(file_data => stripFilePath(file_data.filename) === `${shp_name_without_ext}.dbf`);
    const prj_dataFile = zipFileEntries.find(file_data => stripFilePath(file_data.filename) === `${shp_name_without_ext}.prj`);
 ...
 */
function displayElemTemporarily(elem, seconds=5) {
...
async function readZipFile(evt) {
  ...
  try {
    ...
    try {
      ...
      const notAZipError = new Error("No zip file selected. Select a valid zip file.");
    ...
    if (!groupedByName.length) {
      const invalidZipError = new Error("No shapes (.sph) detected in zipfile.");
...
export function setUpZipFileHandler() {
  document.getElementById("shapefiles-file-input")
