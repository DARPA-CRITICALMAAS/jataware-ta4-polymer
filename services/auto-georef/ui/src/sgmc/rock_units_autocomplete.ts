...
function renderRockUnitBadges() {
    ...
    selectedItems.forEach((i, idx) => {
        ...
        colorPicker.className = "w-6 bg-transparent cursor-pointer";
        colorPicker.value = `${i.color || "ffffff"}`; // Set the color if it exists, otherwise default to white

        const badgeContent = document.createElement("div");
        badgeContent.classList.add("flex", "items-center", "gap-1");
        ...
        badgeContent.insertBefore(colorPicker, badgeContent.firstChild);

        newBadge.appendChild(badgeContent);
        ...
        colorPicker.addEventListener("input", (e) => {
            ...
            selectedItems[idx]['color'] = e.target.value
        });
    });
...
function validateButton(selectedItems) {
    if (selectedItems.length == 0) {
