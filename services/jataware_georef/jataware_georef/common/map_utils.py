...
def create_buffered_map_area(points: List) -> Area_Extraction:
    min_rowb = max_rowb = round(points[0]["rowb"])
    min_coll = max_coll = round(points[0]["coll"])

    for point in points[1:]:
        min_rowb = min(min_rowb, round(point["rowb"]))
        max_rowb = max(max_rowb, round(point["rowb"]))
        min_coll = min(min_coll, round(point["coll"]))
        max_coll = max(max_coll, round(point["coll"]))
    ...
    polygon_coordinates = [
        [
            [int(min_coll), int(min_rowb)],
            [int(max_coll), int(min_rowb)],
            [int(max_coll), int(max_rowb)],
            [int(min_coll), int(max_rowb)],
            [int(min_coll), int(min_rowb)],
        ]
    ]

    return Area_Extraction(
...
def inverse_geojson(geojson, image_height):
    ...
    if geom_type == "Point":
        ...
        geojson["coordinates"] = [
            transform_coord(coord) for coord in geojson["coordinates"]
        ]
    elif geom_type in ["Polygon", "MultiLineString"]:
        geojson["coordinates"] = [
            [transform_coord(coord) for coord in ring]
            for ring in geojson["coordinates"]
        ]
    elif geom_type == "MultiPolygon":
        geojson["coordinates"] = [
            [[transform_coord(coord) for coord in ring] for ring in polygon]
