...
import { Checkbox, Button, TextField, Table, TableBody, TableRow, TableCell, Paper, TablePagination, TableHead } from '@mui/material';
...
function Landing() {
    ...
    function remove_old_clip() {
        let feature_source = getLayerById(mapRef.current, "bounding-box").getSource()
    ...
    const paginateMaps = async () => {
        try {
            ...
            let features_source = getLayerById(mapRef.current, "bounding-box").getSource()
            ...
            if (bbox !== undefined) {
                ...
                if (bbox.getGeometry().getType() == "Polygon") {
                    ...
                    destCoordinates = transformPolygonCoordinates(sourceCoordinates, "EPSG:3857", "EPSG:4326");
                    convertedGeojsonFeature.geometry.coordinates = destCoordinates;
                    ...
                    for (let poly of sourceCoordinates) {
                        new_coords.push(transformPolygonCoordinates(poly, "EPSG:3857", "EPSG:4326"))
                    }
            ...
            var data = {
                ...searchParams,
                bounding_box_polygon: convertedGeojsonFeature,
            ...
            axios({
                ...
                let map_layer_source = getLayerById(mapRef.current, "found-maps").getSource()
    ...
    useEffect(() => {
        const dict_ = scaleMarks.reduce((acc, item) => {
            acc[item.value] = parseInt(item.label.split(":")[1].replace(/,/g, ''), 10)
            return acc;
        }, {});
    ...
    useEffect(() => {
        ...
        for (let d of state_data.features) {
            ...
            if (d.geometry.type == "Polygon") {
                ...
                destCoordinates = transformPolygonCoordinates(sourceCoordinates, "EPSG:4326", "EPSG:3857");
                d.geometry.coordinates = destCoordinates;
            } else if (d.geometry.type == "MultiPolygon") {
                sourceCoordinates = d.geometry.coordinates;
                let new_coords = []
                for (let poly of sourceCoordinates) {
                    new_coords.push(transformPolygonCoordinates(poly, "EPSG:4326", "EPSG:3857"))
                }
    ...
    useEffect(() => {
        ...
        document.title = "Polymer Georeferencer";
        loadStats()
    ...
    function checkObjectForFalseValues(inputObject) {
        ...
        if (allFalse) {
            alert("Alert: Please select at least one Map type (Georeferenced, Validated, or Not Georeferenced) ");
            return true;
        }
        return false
    }
    ...
    function transformExtent(coordinates, source, dest) {
        let coordinates1 = transform([coordinates[0], coordinates[1]], source, dest)
        let coordinates2 = transform([coordinates[2], coordinates[3]], source, dest)
            ...
            <>
                <div className="Landing2" >
                    <div className="header">
                        <h1 className="title">Georeferencer</h1>
                                ...
                                <div className="card-grid">
                                    <div className="card2">
                                        <div className="card-title">
                                            <span style={{
                                                backgroundColor: returnRowColor({ "validated": false, "georeferenced": true }),
                                                borderRadius: "5px",
                                                padding: "3px"
                                            }}>
                                                Georeferenced
                                            </span></div>
                                        <div className="card-count">Count: {statCounts['georeferenced']}</div>
                                        <button onClick={() => navigateToMap('georeferenced')}>View</button>
                                    </div>

                                    <div className="card2">
                                        <div className="card-title">
                                            <span style={{
                                                backgroundColor: returnRowColor({ "validated": false, "georeferenced": false }),
                                                borderRadius: "5px",
                                                padding: "3px"
                                            }}>
                                                Not Georeferenced
                                        ...
                                        <div className="card-count">Count: {statCounts['not_georeferenced']}</div>
                                        <button onClick={() => navigateToMap('not_georeferenced')}>View</button>
                                    </div>

                                    <div className="card2">
                                        <div className="card-title">
                                            <span style={{
                                                backgroundColor: returnRowColor({ "validated": true, "georeferenced": false }),
                                                borderRadius: "5px",
                                                padding: "3px"
                                            }}>
                                                Validated
                                        ...
                                        <div className="card-count">Count: {statCounts['validated']}</div>
                                        <button onClick={() => navigateToMap('validated')}>View</button>
                                    </div>
                                    <div className="card2">
                                        <div className="card-title">
                                            <span style={{
                                                backgroundColor: returnRowColor({ "not_a_map": true }),
                                                borderRadius: "5px",
                                                padding: "3px"
                                            }}>
                                                Not A Map
                                        ...
                                        <div className="card-count">Count: {statCounts['not_a_map']}</div>
                                        <button onClick={() => navigateToMap('not_a_map')}>View</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className='mapSearch'>
                        ...
                        <div className='search_group'>
                            ...
                            <div className='checkbox-grid '>
                                <div>
                                    ...
                                    />
                                    <div onClick={() => { handleStateSelection("") }}
                        ...
                        <div style={{ display: "grid", width: "100%", display: "flex" }}>
                            ...
                            <div style={{ minWidth: "30%" }}>
                                <div style={{ overflowY: "scroll", height: "545px" }}>
                                    <TablePagination
                                        component="div"
                                    ...
                                    {mapsList.map((item, index) => (
                                        <MapScrollCard key={index}
                                            returnRowColor={returnRowColor}
                                            item={item}
                                            updateMap={updateMapHighlight}></MapScrollCard>
                                    ))}
                                </div>
...
function MapScrollCard({ returnRowColor, item, updateMap }) {
    ...
    function handleMapClick(map_info, path) {
        window.open("./" + path + "/" + map_info['map_id']);
    }


    return (
        <div key={item.map_id} style={{ color: "black", backgroundColor: returnRowColor(item), border: '1px solid black', padding: '4px', borderRadius: '5px', display: 'flex', flexDirection: 'column' }}>
            <h5 style={{ margin: "4px" }}><span style={{ color: "black", fontSize: "15px" }}> {item['title']}</span></h5>
            <p style={{ margin: "4px" }}><strong>Year:</strong> <span style={{ color: "black" }}>{item['year']}</span></p>
            <p style={{ margin: "4px" }}><strong>Author:</strong> <span style={{ margin: "4px", color: "black" }}>{item['authors']}</span></p>
            ...
            {info &&
                ...
                <div style={{
                    ...
                    <p style={{ margin: "4px" }}><strong>Map ID:</strong> <span style={{ color: "black" }}>{item['map_id']}</span></p>
                    <p style={{ margin: "4px" }}><strong>Map Name:</strong> <span style={{ margin: "4px", color: "black" }}>{item['map_name']}</span></p>
                    <p style={{ margin: "4px" }}><strong>Category:</strong> <span style={{ margin: "4px", color: "black" }}>{item['category']}</span></p>
                    <p style={{ margin: "4px" }}><strong>Series Name:</strong> <span style={{ margin: "4px", color: "black" }}>{item['series_name']}</span></p>
                    <p style={{ margin: "4px" }}><strong>Publisher:</strong> <span style={{ margin: "4px", color: "black" }}>{item['publisher']}</span></p>
                    <p style={{ margin: "4px" }}><strong>Publication Link:</strong> <span style={{ margin: "4px", color: "black" }}>{item['pub_link']}</span></p>
                </div>
            }
